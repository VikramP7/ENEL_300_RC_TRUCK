...
int main(void)
{
    ...
    {
        /* -------- Bluetooth Comms ------------*/
        // transmit to remote side, send distance and metal detection data
        uint8_t transmitPackage = (distanceMeasured & 0xfe) | (metalDetected);
        BluetoothTransmit(transmitPackage);

        _delay_ms(1);

        // receive data from remote, left and right motor speed, and light signal
        ...
        // toggle the heart beat every super loop
        if (heartBeatOn)
        {
            LedOff(LED_DEBUG_RED);
            heartBeatOn = 0;
